...
Native processing and reusable composition intermediates remain scene-linear, with half-/full-float precision as required. The viewer playback cache is a separate, **display-referred BC7 RGBA** representation of the selected-resolution result after its viewer color transform (owner decision #106). Store explicit viewing/color-config identity, alpha association, logical dimensions, pixel aspect, signed region and sampling lattice. Use `VK_FORMAT_BC7_UNORM_BLOCK` for the already display-encoded samples: do not introduce an sRGB sampling decode, equate Rec.709 and sRGB labels, or apply the baked viewer transform twice.

BC7 is a lossy SDR display approximation, distinct from resolution approximation. Alpha survives compression; deterministic block-edge padding must not change the logical geometry or reveal padded texels. The representation must not substitute for scene-linear node inputs, precise source-value inspection, or full-quality delivery. UI overlays, handles, guides and other presentation-only decorations remain outside the encoded image.
...
Construct BC7 blocks with Vulkan compute through the existing GPU owner, then transfer them into sampleable compressed images; BC7 storage-image support is not required. Persistence reads back only completed compressed blocks. Independent frame records live in bounded indexed pack files, with complete metadata/payload/integrity validation before durable publication. There is no video decoder, GOP dependency, YUV conversion, CPU encoder or alternate cache codec. Unsupported device capabilities leave live rendering available with an identifying cache diagnostic.

Known valid frame/representation records bypass graph description and dependency planning as well as evaluation. A loading entry coalesces with its asynchronous read/upload rather than becoming a live-render miss. Direction-aware read-ahead visits existing cached entries only. Bounded ready textures overlap preparation and presentation; ordered transport readiness is distinct from latest-wins seek/edit publication. Reverse, loop, seek, stop and destination closure cannot publish obsolete work or release in-flight resources early. Replay samples BC7 directly into the existing shared RGBA8 surface and external-memory/semaphore handoff to Qt; it does not reconstruct a float working image.
...
Use a budgeted disk-backed cache with a bounded compressed RAM hot set and a small compressed GPU ready queue. Retain valid entries while budgets permit, evicting least-recently-used eligible entries before allocation; eviction removes residency, whereas invalidation means a result no longer matches its dependencies. Reserve the active working set through the shared allocator, and charge actual allocation requirements, staging, retained live inputs, in-flight work, presentation, metadata and disk storage. Physical shared presentation memory is charged once. Active resources stay pinned until completion. Reclaim dead pack storage within the disk budget without touching source, project or export files. Explicit clearing removes selected cache-owned storage; neither eviction nor clearing modifies the graph.
...
Named channels retain their names and values through media, evaluation and native storage. Ordinary RGBA effects preserve auxiliary channels without changing their established RGBA arithmetic. Only a complete identified primary/root RGB set receives color conversion; alpha-only and auxiliary/data samples bypass it. A viewer's individual data-channel selection displays that channel as opaque gray, not an invented color or alpha role.

Shuffle routes up to eight authored output sockets from two selectable input groups or zero/one constants while preserving untouched B channels. B is required; A is optional. Missing source names, disconnected A and explicitly unwired outputs produce zero. Only authored output names create channels, with exact names preserved. Constants cover B's logical format and A retains its own coordinates. Mapping edits, animation, exposure, persistence and unavailable-editor recovery use the existing contribution/parameter/command contracts.
...
| Viewer playback cache | GPU BC7 construction with explicit display/color/alpha interpretation, no second transform, and declared lossy pixel tolerances; requested frames become visible before asynchronous caching. Independent close/reopen/forward/reverse/random replay, validated hits with zero graph planning/evaluation, loading coalescence, invalidated-frame replacement, bounded pressure/cancellation and obsolete-write rejection are demonstrated on the native surface. |
