...
| Graph geometry (single owner) | `apps/nemo-ui/GraphGeometry.hpp`/`GraphGeometry.cpp`; `cardRect`, `portPosition`, `affordanceRect`/`withinAffordance`, `portSide`/`portGuardSatisfied`, `routePolyline`, `projectOnPolyline`, `contentBounds`, `GraphViewTransform`, the `k*` preserved constants | The painter and the picker both call these; `portPosition` is the one place a port centre is computed, and a second implementation of card, port, route or projection geometry is a defect. Tolerances are screen-space and converted through the same transform. ADR-0009 |
