...
| Viewer display cache and replay | `eval/ViewerCache.*`, `eval/Viewer.*`, `eval/ViewerScheduler.*`; `gpu/Bc7.*` and `gpu/ViewerPresentation.*` | #106 replaces #12 video chunks with independent BC7 RGBA pack records. Cache owns bounded storage/readiness; Session owns validated per-frame identity/description; Scheduler/runtime own ordered preparation versus latest-wins publication. GPU owns encoding, compressed transfer and direct sampling into the existing shared RGBA8 handoff. Follow `rendering.md`'s viewer replay integration; never pass BC7 to effect/source/picker/delivery interfaces. |
| GPU byte admission and working-set reservations | `gpu/Allocator.*` with completion-retained allocation tokens | The narrow #97 prerequisite consumed by #106 extends this owner with actual allocation charges, reservations and registered eligible eviction. Cache tiers contribute eligible residency; active resources stay charged/pinned until their final completion owner releases them. Shared presentation memory is physically charged once; no cache-local allocator or count-as-byte accounting. |
...
| Roto shapes and authoring | `core/document/Roto.*`, `core/commands/RotoCommands.*`, `nodes/roto/`; typed immutable hierarchy plus existing animation addresses | `RotoController` is a transient per-network/node/context-group adapter. `RotoEditor.qml` uses the shared numeric/key/history controls; `RotoOverlay.qml` uses the viewer's existing image mapping. Drafts and selection are transient; authored changes publish through `ProjectSession`. Numerical and bounded-geometry policy lives in ADR-0008; schema and reuse in ADR-0007 |
...
| Native file chooser | `apps/nemo-ui/NativeFileChooser.hpp`; `openFiles`/`saveFile`/`openFolder` with a requester-owned `OutcomeHandler` | One platform implementation per build; each request belongs to its requester. Project, media, delivery and Settings controllers borrow the same chooser; node browse results retain the requesting network/node identity |
...
| Shared node appearance | `WorkspaceController::categoryColors`, consumed through `Theme.qml` | Keys are exact catalog groups, shared by graph, inspector and appearance settings; catalog menus retain those same group identities. Appearance version 3 migrates version-2 custom colors and updates untouched defaults (#101). Add category styling here, not in individual nodes or panels. Accepted appearance and Settings/cache edits auto-save to the existing workspace application-preference store (`appearance` and `settings` records), and roll back with a visible error if the write fails; `projectPresentation()` carries arrangement records only, so a project never overwrites or imports them. |
| Application Settings | `SettingsWindow.qml` and its section components; `apps/nemo-ui/SettingsController.*` | Settings is a separate non-modal window, not a dock panel. `WorkspaceController` persists window/appearance/cache-storage preferences; `InstalledPackages` owns discovery, trust preferences and next-launch admission. Settings only adapts those owners and the existing viewer cache snapshot; unavailable #14 budget/clear capabilities stay explicit rather than acquiring another cache manager. Neither preference path edits a project. |
...
| Graph scene records | `apps/nemo-ui/GraphScene.hpp`/`GraphScene.cpp`; `buildGraphScene`, `GraphScene`, `GraphNodeRecord`/`GraphPortRecord`/`GraphEdgeRecord`/`GraphEndpointRecord`, the `node`/`edge` identity index and `revision` | Built on the GUI thread from `ViewerController::graphSnapshot()`; the paint item, the hit test and the gesture sessions all read this one parsed record set. Immutable, no Qt Quick types, no transient state, never mutated downstream. ADR-0009 |
| Graph geometry (single owner) | `apps/nemo-ui/GraphGeometry.hpp`/`GraphGeometry.cpp`; `cardRect`, `portPosition`/`portArrow`, `affordanceRect`/`withinAffordance`, `portSide`/`portGuardSatisfied`, `routePolyline`, `projectOnPolyline`, `contentBounds`, `GraphViewTransform`, the `k*` constants | Arrow centres protrude outside compact cards (#101); wires and picking share those anchors. Mask glyphs are smaller without reducing screen-space acquisition tolerance. The painter consumes this geometry; a second implementation of card, port, route or projection geometry is a defect. ADR-0009 |
| Graph picking and hover feedback | `apps/nemo-ui/GraphHitTest.hpp`/`GraphHitTest.cpp`; `hitTestGraph(scene, view, screen)`, `GraphHitKind`, `GraphHitResult::primary`, `GraphHover` | One ordered C++ pass per pointer move, screen-space tolerance, topology priority port → connection endpoint → reroute dot → card → pipe body, a port over its own card and a subnet card's enter affordance over everything. `GraphInteraction` publishes hover from the same result a press resolves with; `hitTestPasses` counts the passes. ADR-0009 |
| Graph interaction owner (panel-facing) | `apps/nemo-ui/GraphInteraction.hpp`/`GraphInteraction.cpp`; QML element `GraphInteraction`, `objectName: graphInteraction`; owns the scene, view transform, selection, hover and the active session | The only object the graph panel and the paint item talk to. `setSnapshot` aborts a live session on a different revision instead of rebasing it; `viewSettled`, `enterAffordances`, `nodeRect`/`portPosition`/`scenePoint` are the records panel chrome positions itself from. ADR-0009 |
| Graph gesture sessions | `apps/nemo-ui/GraphSessions.hpp`/`GraphSessions.cpp`; `GraphSession` (`historyGesture`, `acceptsWheel`, `moved`, `preferOutputEndpoint`, `preview`, `update`, `cancel`, `commit`), `GraphPreview`, and `MoveSession`/`ToggleSession`/`MarqueeSession`/`ConnectSession`/`RerouteSession`/`ScopeEnterSession`/`ViewSession` | Begin is constructing the session; cancellation drops it and never reaches the command facade or history. `GraphSessionContext` carries this event's own ordered pass, so a session never resolves a second pick. One session per gesture, no string-typed state or loose flag properties; `ViewSession` owns the view transform, the per-turn zoom application and the settle timer. A new behavior is a new session type, never a branch in a shared handler. ADR-0009 |
| Graph command facade | `apps/nemo-ui/GraphCommandFacade.hpp`/`GraphCommandFacade.cpp`; `moveNodes`, `connectOrReplace`, `rewire`, `disconnect`, `commitRoute`, `insertExistingNodeOnEdge`, `createNode`, `deleteNodes`, `assignViewer`, `collapseSelection`, `duplicateLinked`, `makeIndependent`, `copySelection`, `pasteSelection` | The only place in the interaction layer that calls the graph command API, so one call site covers every UI commit and automation keeps sharing that path. A command whose refusal matters returns it (`insertExistingNodeOnEdge`, `createNode`, `deleteNodes`, `collapseSelection`, `copySelection`, `pasteSelection`, `duplicateLinked`, `makeIndependent`); an edit nothing acts on returns nothing and its refusal is visible through the document. ADR-0009 |
| Graph paint item | `apps/nemo-ui/GraphItem.hpp`/`GraphItem.cpp`; `GraphItem::updatePaintNode`, `labelAtlasesRasterized` | Consumer only: it draws the interaction's scene, the shared geometry and the active session's `GraphPreview`, computes no layout, holds no interaction state and re-parses no snapshot. The label atlas identity is the label set and the device pixel ratio, never a view scale or a position. ADR-0009 |
| Graph panel presentation | `apps/nemo-ui/qml/GraphPanel.qml` over the `graphInteraction` object | Input plumbing, chrome, popups and shortcuts: it computes no graph geometry, iterates no graph elements and resolves no pick, and every coordinate it positions chrome with comes from a record the interaction published. Panel state is written once per settled gesture (`onViewSettled: graphPanel.savePanelState()`); authored positions stay in the document. ADR-0009 |
...
`extensions/InstalledPackages` owns installed-package discovery, metadata
inspection (`inspectInstalledPackages`, which never executes a package), the
per-user enablement/linked-folder settings and activation; the Settings surface
consumes that API and never parses a manifest itself.
