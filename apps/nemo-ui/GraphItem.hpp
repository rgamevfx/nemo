...
//
// The scene graph owns every QSG resource. Nothing is re-rasterised because a
// node moved: the label atlas identity is the label set and the device pixel
// ratio, never the view scale or a position, so a drag moves quads rather than
// re-uploading a texture.
